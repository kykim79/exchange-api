...
public class GetDailyBTCChartDataExample {
    private final static Logger LOGGER = LoggerFactory.getLogger(GetDailyBTCChartDataExample.class);
    ...
    public void run(String propertiesFileName) {
        ...
        if (tradingAPIKey == null) {
            LOGGER.warn("Did not find value for " + POLONIEX_API_KEY_PROP_NAME + " in " + propertiesFileName + ". Trading API commands will fail");
        }

        String tradingAPISecret = properties.getProperty(POLONIEX_API_SECRET_PROP_NAME);
        if (tradingAPISecret == null) {
            LOGGER.warn("Did not find value for " + POLONIEX_API_SECRET_PROP_NAME + " in " + propertiesFileName + ". Trading API commands will fail");
        }
    ...
    private Properties loadProperties(String propertiesFileName) {
        ...
        try (InputStream in = this.getClass().getClassLoader().getResourceAsStream(propertiesFileName)) {
            ...
            LOGGER.error("Could not load properties file " + propertiesFileName + " - " + ex.getMessage());
        }
